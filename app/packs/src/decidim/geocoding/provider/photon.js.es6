            ...
            if (resp.features) {
              return callback(resp.features.map((item) => {
                ...
                return {
                  key: label,
                  value: label,
                  // THE ONLY CHANGE TO THIS FILE IS THE FOLLOWING LINE
                  // this line is a bug in decidim, the photon API gives coordinates as [long, lat],
                  // whereas everywhere in decidim, we work with [lat, long]
